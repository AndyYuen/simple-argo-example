...
mkdir $DIR
cd $DIR
echo "schemaUrl: https://raw.githubusercontent.com/andyyuen/kafka-sizing/master/src/main/resources/schema-mysql.sql" > values.yaml
...
cp $SCRIPT_DIR/../helm-kafka-sizing/templates/_mysql-hook.yaml templates/
cp $SCRIPT_DIR/../helm-kafka-sizing/Chart.yaml .
cp $SCRIPT_DIR/../helm-kafka-sizing/values.yaml .
